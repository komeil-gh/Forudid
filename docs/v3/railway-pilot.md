...
## Completed local handoff — 2026-09-11

Alpha.11 completes the available software walkthrough through report generation
and file delivery. Local review material is prepared under
`data/pilots/railway-1093520814-alpha11/`, with an adjacent ZIP. It contains the
four-page Persian PDF, full analysis JSON, all 120 profile samples in CSV, both
intervals in GeoJSON, original asset geometry, product/STAC metadata, report
manifest/status, an unfilled review record and file checksums. It is excluded
from Git and from the public source archive.

The profile CSV matches every archived sample, and both interval lengths sum to
the full way length. Report `e23ce257-284a-56cd-8ef1-201de9ae89d2` pins this exact
analysis. PDF SHA-256 is
`ba563958fae78d9847581ebc14dcee41ed4cf963560e4cc747e0ab1e36a8cdb2`.
All four rendered pages were visually inspected. All 12 handoff files match the
ZIP contents; its SHA-256 is
`97dbe645c787483f7a0bf51d5873bc536aef2f04b9ae18778f7c1542463c0f94`.

The asset page can open the same interval in the full map without changing the
product or analysis. A failed or nonexistent interval has an explicit recovery
state; it is never silently replaced by the full way. Ranking CSV exports retain
the component, dates and sign alongside the values. These are completed software
steps, not tasks delegated to the project owner. Participant recruitment, actual
decisions and field outcomes remain external evidence that cannot be generated by
