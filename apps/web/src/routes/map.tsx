...
import { useListProducts, useGetLegend, useGetInfrastructureAsset, useGetRegion, type ProfileSample } from '../generated/api/forudid'
...
export default function MapPage() {
  ...
  function selectPoint(lon: number, lat: number) {
    if (!product) return
    update({ pointLon: roundCoordinate(lon), pointLat: roundCoordinate(lat), panel: 'point', asset: undefined, analysis: undefined,
  ...
  const controls = state.mode === 'deformation' ? <LayerPanel {...panelProps} /> : <>
    <ModePanel state={state} product={product} update={update} selectAsset={(id, runId) => {
      update({ asset: id, analysis: runId, panel: 'asset', pointLon: undefined, pointLat: undefined }); setLayersOpen(false)
    }} />
  ...
  return <main className="map-page"><div className="map-modes" role="group" aria-label={language === 'fa' ? 'حالت نقشه' : 'Map mode'}>
    {(['deformation', 'infrastructure', 'population'] as const).map(mode => <button key={mode} aria-pressed={state.mode === mode} onClick={() => {
      ...
      update({ mode, panel: 'none', asset: undefined, analysis: undefined, pointLon: undefined, pointLat: undefined,
    ...
    <div className="map-workspace"><div className="map-region">
      ...
      {showAsset && <AssetPanel data={asset.data} pending={asset.isPending} error={asset.isError}
        productId={product?.id} runId={state.analysis} onInspect={inspectProfile}
        retry={() => void asset.refetch()} close={() => update({ panel: 'none', asset: undefined, analysis: undefined })} />}
