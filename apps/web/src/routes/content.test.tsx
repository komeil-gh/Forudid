...
describe('scientific content pages', () => {
  ...
  it('renders the supplied dedication and attribution', () => {
    ...
    expect(screen.getByText('فرودید | پایش ماهواره‌ای فرونشست ایران زمین')).toBeInTheDocument()
    expect(screen.getAllByRole('button', { name: /معرفی/ })).toHaveLength(8)
    expect(document.querySelector('.about-binary-watermark pattern text')?.textContent).toMatch(/^[01]+$/)
  ...
  it('renders the English dedication without enlarging the author signature', () => {
    ...
    expect(screen.getByText('Forudid | Satellite monitoring of land subsidence in Iran')).toBeInTheDocument()
