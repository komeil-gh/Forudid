...
export function AssetPanel({ data, pending, error, retry, close, productId, onInspect }: {
  data?: InfrastructureFeature; pending: boolean; error: boolean;
  retry: () => void; close: () => void;
  productId?: string; onInspect: (sample?: ProfileSample) => void;
}) {
  ...
  return <section className="point-panel" aria-label={en ? 'Selected infrastructure' : 'زیرساخت انتخاب‌شده'}>
    ...
    {data && <div className="point-chart">
      {productId && <ExposureDetails key={`${data.id}/${productId}`} assetId={data.id} productId={productId} onInspect={onInspect} />}
