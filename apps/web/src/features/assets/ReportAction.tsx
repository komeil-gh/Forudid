...
import { getGetScreeningReportQueryKey, useCreateScreeningReport, useGetScreeningReport, type ReportRequest } from '../../generated/api/forudid'
import { useLanguage } from '../../i18n'
import { apiBase } from '../../lib/api'
import { Button } from '../../components/ui/button'
import { Status } from '../../components/Status'

export function ReportAction({ request }: { request: ReportRequest }) {
  ...
  return <section aria-label={en ? 'Persian screening report' : 'گزارش غربالگری فارسی'}>
    {job?.status === 'completed' ? <p><a href={`${apiBase}/api/v1/reports/${job.id}/download`}>{en ? 'Download Persian screening PDF' : 'دانلود گزارش غربالگری فارسی (PDF)'}</a></p> : <Button
      disabled={busy} onClick={() => create.mutate({ data: request })}>
