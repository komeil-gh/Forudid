...
test('real ranking opens a pinned asset profile and downloads real CSV', async ({ page, isMobile }) => {
  ...
  const selectedGeometry = page.waitForResponse(r => /\/segments\?offset=1&limit=1$/.test(r.url()) && r.ok())
  await page.locator('.segment-table tbody button').nth(1).click()
  expect((await (await selectedGeometry).json()).features[0].properties.ordinal).toBe(1)
  await expect(page).toHaveURL(/segment=1/)
  await expect(page.locator('.segment-table tbody button').nth(1)).toHaveAttribute('aria-pressed', 'true')
  await page.reload()
  await expect(page.getByTestId('exposure-mean')).toHaveText('۹۳٫۵ mm/year')
  await page.getByText('بازه‌های مواجهه', { exact: true }).click()
  await expect(page.locator('.segment-table tbody button').nth(1)).toHaveAttribute('aria-pressed', 'true')
  await page.getByText('مقادیر نمونه‌ها', { exact: true }).click()
  await page.locator('.series-table:not(.segment-table)').getByRole('button').first().click()
  await expect.poll(async () => {
    const marker = await page.getByTestId('profile-map-marker').boundingBox(), map = await page.locator('.asset-preview').boundingBox()
    return !!marker && !!map && marker.x+marker.width/2 >= map.x && marker.x+marker.width/2 <= map.x+map.width && marker.y+marker.height/2 >= map.y && marker.y+marker.height/2 <= map.y+map.height
  }).toBe(true)
  await page.getByText('مقادیر نمونه‌ها', { exact: true }).click()
  await page.getByRole('button', { name: 'نمایش کل قطعه', exact: true }).click()
