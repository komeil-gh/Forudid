...
test('historical COG tile, source measurement and stable reload', async ({ page, isMobile }, testInfo) => {
  ...
  await page.goto('/map?lon=55.65&lat=30.87&z=10&infrastructure=none')
...
test('QC/point failure never leaves an unqualified velocity value', async ({ page }) => {
  ...
  await expect(page.getByText(/^(?:370\.0|۳۷۰٫۰)$/)).toHaveCount(0)
  await expect(page.locator('.maplibregl-canvas')).toBeVisible()
})

test('native pixel inspection preserves precise coordinates, map space and missing coverage', async ({ page, isMobile }, testInfo) => {
  await page.goto(`${golden}&infrastructure=none`)
  const panel = page.getByRole('region', { name: 'نقطهٔ انتخاب‌شده' })
  await expect(panel.getByText(/^(?:370\.0|۳۷۰٫۰)$/)).toBeVisible()
  const mapBox = (await page.locator('.maplibregl-canvas').boundingBox())!
  expect(mapBox.height).toBeGreaterThan(isMobile ? 330 : 500)
  await page.getByRole('button', { name: 'نمای سلول بومی', exact: true }).click()
  await expect.poll(() => Number(new URL(page.url()).searchParams.get('z'))).toBeGreaterThan(14)
  await panel.locator('.native-cell-details summary').click()
  await expect(panel.locator('.native-cell-details')).toContainText('55.653687, 30.869620')
  await expect(page.locator('.native-cell-marker')).toBeVisible()
  await page.screenshot({ path: `/tmp/forudid-qa/native-cell-${testInfo.project.name}.png` })
  await panel.getByRole('button', { name: 'بستن', exact: true }).click()
  await expect(page.getByRole('button', { name: 'نمای سلول بومی', exact: true })).toHaveCount(0)
  await expect(page.locator('.point-marker')).toHaveCount(0)
  await page.goto('/map?lon=55.653687451&lat=30.869620061&z=14&infrastructure=none')
  await expect(page.locator('.legend')).toBeVisible()
  await page.getByRole('button', { name: 'بررسی نقطهٔ مرکز نقشه' }).click()
  await expect(page).toHaveURL(/pointLon=55.653687451/)
  await expect(page).toHaveURL(/pointLat=30.869620061/)
  await page.goto('/map?lon=0&lat=0&pointLon=0&pointLat=0&panel=point&infrastructure=none')
  await page.locator('.native-cell-details summary').click()
  await expect(page.locator('.native-cell-details').getByRole('status')).toHaveText('خارج از محدودهٔ رستر منبع.')
  await expect(page.getByRole('button', { name: 'نمای سلول بومی', exact: true })).toHaveCount(0)
})
